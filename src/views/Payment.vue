...
<template>
  ...
  <ul class="w-full">
    <li class="w-full box-border p-[4vw] flex justify-between items-center">
      ...
      <svg :class="selectedPayment === 'alipay' ? 'text-[#38CA73FF]' : 'text-[#a1a1aa]'" @click="togglePayment('alipay')" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
    ...
    <li class="w-full box-border p-[4vw] flex justify-between items-center">
      ...
      <svg :class="selectedPayment === 'wechat' ? 'text-[#38CA73FF]' : 'text-[#a1a1aa]'" @click="togglePayment('wechat')" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
        <path fill="currentColor"
              d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10s10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5l1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
      </svg>
    </li>
  </ul>
    <div class="w-full flex-col h-30 flex box-border pt-[4vw] px-[3vw] pb-0 justify-center items-center" v-for="order in orders" :key="order.orderId">
      <button @click="handlePayment(order.orderId, order.orderState)" class="w-full h-20 text-3xl font-bold text-white bg-[#38CA73FF] rounded-[4px] border-none outline-none">
        确认支付
      </button>
    </div>
  </body>
